...
(def my-bot 
  {:attack
   (fn [x]
     (let [without (remove #(= (% :suit) (x :trump) ) (x :hand))
           ranks (filter #(not= % 15) (map (fn [ar] (if (some #(= (% :rank) ar) (x :hand)) ar 15 ))(sort (map #(% :rank) (x :table)))))]
       (if (empty? (x :table))
           (reduce #(if (< (:rank %) (:rank %2)) % %2) (if (empty? without) (x :hand) without))
         (if (empty? ranks)
           nil
           (first (remove #(not= (% :rank) (first ranks) )  without))
           )
         )))
   :defend
   (fn [x] (let [cards (filter #(higher? % (last (x :table)) (x :trump)) (x :hand))
                 withouttrump (remove #(= (% :suit) (x :tump)) cards)]
      (if (empty? cards) nil (reduce #(if (< (:rank %) (:rank %2)) % %2)
             (if (empty? withouttrump) cards withouttrump ) ))))})
